...
my %stats; 
my $genome2bit;
my $defaultblatopt = "-mask=lower -minIdentity=98 -maxIntron=500";
my $defaultblastopt = "-task blastn -outfmt 6 -strand plus -gapopen 2 -gapextend 1 -reward 1 -penalty -2";
...
sub process_experiment ($$) {
	...
	align_to_genome($expt_id,$expt_hash,$blatopt);
 	...
 	make_tlxl_blast($expt_id,$expt_hash);
...
sub check_existance_of_files {
	...
	foreach my $expt_id (sort keys %meta_hash) {
		...
		my @exts = qw(.fa .fasta .fna .fq .fastq);
		foreach my $ext (@exts) {
			if (-r $file.$ext) {
				if ($ext =~ /q/) {
					(my $next = $ext) =~ s/q/a/;
